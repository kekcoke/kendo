...
## Phase Outputs — Day 23
> Legend: ✅ complete · ❌ failed/blocked · ~ pending · ⏳ deferred

| Phase | Artifact | Status |
|---|---|---|
| 0 | State initialized, variables resolved → M5.4 Gateway → FastAPI routing | ✅ |
| 0b | *Skipped* (repo has prior commits) | ✅ |
| 1 | `docs/architecture/day_23_spec.md` | ✅ |
| 2 | Commit log — 3/3 units committed, zero halted — feature branch on `origin` | ✅ |
...
* **Orchestration — Dependency-Aware Milestone Ordering (Phase 0):** The Phase 0 scope derivation scans all `~` candidates and picks the first whose dependencies are satisfied, rather than strictly picking the first `~` row in the roadmap table. This enables W5 (M5.11) to ship before W3 (M5.9) when W3 depends on `user_embeddings` that W5 creates. Selection rationale is logged to ADL when the chosen milestone differs from the table's first `~`. *(Day 32, reconciliation session)*
* **Orchestration — Prior-Day Sealing Gate (Phase 0):** Before advancing `current_day`, Phase 0 runs `validate_state.sh` for the prior day and checks that Phase 5 wrote `## Completed Days`, `current_phase: 0`, and `incomplete_tasks: []`. A failing prior-day seal is a hard blocker. *(Day 32, reconciliation session)*
* **Orchestration — Documentation Gap Scan (Phase 0, non-blocking):** Each Phase 0 scans all `## Completed Days` rows for missing review reports or runbooks. Gaps are surfaced as `mild` carry-forward items but do not block day advancement. *(Day 32, reconciliation session)*
